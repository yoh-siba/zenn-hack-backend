...
from src.models.types import PartOfSpeech, PromptForImagenByGemini
...
def generate_prompt_for_imagen(_content: str) -> list[MeaningSchema]:
    try:
        response = request_gemini_json(
            _contents=_content, _schema=list[PromptForImagenByGemini]
        )
        if response is None:
        ...
        for item in response:
            if not isinstance(item, PromptForImagenByGemini):
                raise ValueError("Item is not of type PromptForImagenByGemini")
...
meaning_idには、definitionの値をそのまま入れてください。
