...
async def update_flashcard(
    ...
    try:
        # WordsAPIから単語情報を取得
        words_api_response: WordsAPIResponse = await request_words_api(_word)

        # 翻訳用のコンテンツを作成
        content = f"""
英単語「{_word}」の英語の各定義（以下のデータのdefinition）の値について、それぞれ対応する一言の簡潔な日本語訳を考えてください。
...
"""
        ...
        # 英単語「{word}」の{meaning.pos}としての意味「{meaning.definition}」を表現するために、
        ...
        # 単語と意味をFirestoreに保存
        success, error, word_id = create_word_and_meaning(word_instance, meanings)
