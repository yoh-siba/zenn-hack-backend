...
def create_user_doc(user_instance: UserSchema) -> Tuple[bool, Optional[str], Optional[str]]:
    try:
...
def update_user_doc(user_id: str, user_instance: UserSchema) -> Tuple[bool, Optional[str]]:
    try:
...
def read_user_doc(user_id: str) -> Tuple[Optional[UserSchema], Optional[str]]:
    try:
        doc_ref = db.collection("users").document(user_id)
        doc = doc_ref.get()
        if doc.exists:
...
def read_user_docs(user_ids: list[str]) -> Tuple[list[UserSchema], Optional[str]]:
    try:
        if not user_ids:
            return [], "ユーザーIDが指定されていません"
            
        docs = db.collection("users").where("__name__", "in", user_ids).get()
        users = []
        for doc in docs:
