...
async def create_media_doc(media_instance: MediaSchema) -> Tuple[bool, Optional[str], Optional[str]]:
    try:
        doc_ref = db.collection("media")
        new_doc = await doc_ref.add(media_instance.to_dict())
...
async def update_media_doc(media_id: str, media_instance: MediaSchema) -> Tuple[bool, Optional[str]]:
    try:
        doc_ref = db.collection("media").document(media_id)
        await doc_ref.update(media_instance.to_dict())
        return True, None
    except Exception as e:
...
async def read_media_docs(media_ids: list[str]) -> Tuple[list[MediaSchema], Optional[str]]:
    try:
        if not media_ids:
