...
async def create_flashcard_doc(flashcard_instance: FlashcardSchema) -> Tuple[bool, Optional[str], Optional[str]]:
    try:
        doc_ref = db.collection("flashcards")
        new_doc = await doc_ref.add(flashcard_instance.to_dict())
...
async def update_flashcard_doc(flashcard_id: str, flashcard_instance: FlashcardSchema) -> Tuple[bool, Optional[str]]:
    try:
        doc_ref = db.collection("flashcards").document(flashcard_id)
        await doc_ref.update(flashcard_instance.to_dict())
        return True, None
    except Exception as e:
...
async def read_flashcard_doc(flashcard_id: str) -> Tuple[Optional[FlashcardSchema], Optional[str]]:
    try:
        doc_ref = db.collection("flashcards").document(flashcard_id)
...
async def read_flashcard_docs(flashcard_ids: list[str]) -> Tuple[list[FlashcardSchema], Optional[str]]:
    try:
