...
def create_media_doc(media_instance: MediaSchema) -> Tuple[bool, Optional[str], Optional[str]]:
    try:
...
def update_media_doc(media_id: str, media_instance: MediaSchema) -> Tuple[bool, Optional[str]]:
    try:
...
def read_media_doc(media_id: str) -> Tuple[Optional[MediaSchema], Optional[str]]:
    try:
        doc_ref = db.collection("media").document(media_id)
        doc = doc_ref.get()
        if doc.exists:
...
def read_media_docs(media_ids: list[str]) -> Tuple[list[MediaSchema], Optional[str]]:
    try:
        if not media_ids:
            return [], "メディアデータIDが指定されていません"
            
        docs = db.collection("media").where("__name__", "in", media_ids).get()
        media_list = []
        for doc in docs:
            media_list.append(MediaSchema.from_dict(doc.to_dict()))
        return media_list, None
    except Exception as e:
        error_message = f"メディアデータの一括読み込み中にエラーが発生しました: {str(e)}"
        print(f"\n{error_message}")
