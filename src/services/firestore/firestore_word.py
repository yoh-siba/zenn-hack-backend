...
def create_word_doc(word_instance: WordSchema) -> Tuple[bool, Optional[str], Optional[str]]:
    try:
...
def update_word_doc(word_id: str, word_instance: WordSchema) -> Tuple[bool, Optional[str]]:
    try:
...
def read_word_doc(word_id: str) -> Tuple[Optional[WordSchema], Optional[str]]:
    try:
        doc_ref = db.collection("words").document(word_id)
        doc = doc_ref.get()
        if doc.exists:
...
def read_word_docs(word_ids: list[str]) -> Tuple[list[WordSchema], Optional[str]]:
    try:
        if not word_ids:
